...
const ContactMe = () => {
  ...
  const [step, setStep] = useState(() => {
    // If already submitted, start directly at success state
    return localStorage.getItem('portfolio_contacted') === 'true' ? 5 : 0;
  });
              ...
              <div className="flex h-full min-h-[320px] flex-col">
                ...
                <div className="mb-4 flex items-center justify-between border-b border-slate-700/30 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Only show reset button if not yet submitted */}
                    {step !== 5 && (
                      <button
                        onClick={handleReset}
                    ...
                    {step === 5 && (
                      <span className="text-[10px] font-bold uppercase tracking-widest text-green-400/70">Sent</span>
                    )}
                  </div>
                </div>

                {/* Terminal Content */}
                <div id="terminal-content" className={`flex-1 font-mono text-sm space-y-2 custom-scrollbar ${step === 0 || step === 5 ? 'overflow-hidden' : 'overflow-y-auto'}`}>
                  ...
                  {step === 5 && (
                    <div className="h-full flex flex-col items-center justify-center space-y-3 text-center">
                      ...
                      <div className="text-green-400 font-bold text-sm tracking-wide">Message Sent!</div>
                      <div className="text-gray-500 text-xs max-w-[200px] leading-relaxed">
                        I've received your message and will get back to you soon.
                      </div>
                    </div>
                  )}

                  {/* History of interaction — hidden after success */}
                  {step !== 5 && consoleOutput.map((line, index) => (
